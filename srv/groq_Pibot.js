...
bot.command('model', (ctx) => {
  const { context, roles, skills, tasks, process, characteristics, imediateActions } = roleDescription;
...
`
    ...
    const knowleddge = `${racine}+${cdnJs}+${node}+${make}+${archiviste}+${github}+${dchub_public}+${user_Pibot}+${youtube_Pibot}+${google_Pibot}+${gemini_Pibot}+${groqIa}`;
...
console.log(`Server Telegram running ✨.groq-ia_Pibot.`);
bot.launch();
